...
function BaggageOptionsSection() {
    ...
    const deleteLuggage = async (index) => {
        try {
          await fetch(`http://localhost:8080/sitas/v1/luggage/${index}`, {
            method: 'DELETE'
          });
          
          // Actualizar el estado de los equipajes eliminando el equipaje en el índice dado
          setLuggages(prevEquipajes => prevEquipajes.filter((_, i) => i !== index));
        } catch (error) {
                        ...
                        {luggages.length > 0 ? (
                            ...
                            <Table>
                                ...
                                {luggages.map((luggage, index) => (
                                    ...
                                    <TableCell>
                                        <IconButton onClick={() => deleteLuggage(index)}>
                                        <DeleteIcon/>
                                        </IconButton>
                                    </TableCell>
                                    </TableRow>
